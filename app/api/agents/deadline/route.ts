...
import { runDeadlineAgent } from '@/lib/agents/deadline'
import { agentGuard } from '@/lib/agents/agent-auth'

/**
 * POST /api/agents/deadline
...
export async function POST(request: NextRequest) {
  ...
  const guard = await agentGuard(request, 'deadline', firm_id)
  if (!guard.ok) return guard.response!

  const result = await runDeadlineAgent(firm_id)
